...
class SaleController extends Controller
{
            ...
            foreach ($sales as $key=>$sale)
            {
                ...
                else
                {
                    $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="'.route('sale.invoice2', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Invoice').'</a></li>
                                <li><a href="'.route('sale.wo2', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Cetak Surat Jalan</a></li>
                                <!--
                                <li>
                                    <button type="button" class="btn btn-link view"><i class="fa fa-eye"></i> '.trans('file.View').'</button>
                                </li>-->';
    ...
    public function store2(Request $request)
    {
        ...
        if($lims_sale_data->sale_status == '1')
        {
            ...
            return redirect('sales/gen_invoice2/' . $lims_sale_data->id)->with('message', $message);
        }
        elseif($data['pos'])
            return redirect('pos')->with('message', $message);
        else
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if ($last_number->invoice_month == $currentMonth)
            {
                ...
                $faktur = $nextNumber."/".$currentDate."/".$this->integerToRoman($currentMonth)."/".$currentYear."-1";
            }
            else
            {
                $faktur = "001/".$currentDate."/".$this->integerToRoman($currentMonth)."/".$currentYear."-1";
            }
    ...
    public function posSale2()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if ($last_number->invoice_month == $currentMonth)
            {
                $next = $last_number->invoice_number + 1;
                $nextNumber = str_pad($next, 3, "0", STR_PAD_LEFT);
                $faktur = $nextNumber."/".$currentDate."/".$this->integerToRoman($currentMonth)."/".$currentYear."-2";
            }
            else
            {
                $faktur = "001/".$currentDate."/".$this->integerToRoman($currentMonth)."/".$currentYear."-2";
            }
    ...
    public function genInvoice2($id)
    {
        ...
        return view('backend.sale.invoice2', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
    ...
    public function genWo2($id)
    {
        ...
        return view('backend.sale.wo2', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
